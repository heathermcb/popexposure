...
`PopExp` is a package aimed to help environmental epidemiologists with exposure assignment. The goal of `PopExp` is to provide functions to determine the number of people residing near environmental hazards such as wildfires, floods, oil wells, or tropical cyclones that are fast, reproducible, and easy to use.

### I. The details

`PopExp` identifies the number of people affected by an environmental hazard by overlaying environmental hazard geospatial data with gridded population data. The function then counts the number of people that fall within the area of an environmental hazard (or a set of environmental hazards). For example, PopExp can determine the number of people affected by a specific hurricane given spatial information about the area hit by the hurricane and gridded population data. Similarly, PopExp can determine the number of people affected by wildfires in 2024 given spatial information of all areas where wildfires occurred in 2024. As many studies use US census geographies (e.g., census tracts, ZCTAs, etc.), PopExp can also estimate the number of people in the US affected by wildfire disasters from 2015-2020 by ZCTA. _Note: areas affected can be the area of the hazard or a buffered area of the hazard._
...
1. Estimate the number of people affected by an environmental hazard: `find_num_people_affected`
   ...
   **Inputs:**
   ...
   - **Environmental hazard geospatial data**: Path to a geospatial data file (GEOJSON or geoparquet file) containing boundaries of environmental hazards (e.g., wildfire disaster boundaries 2015-2020). Columns must include:
...
2. Estimate the number of people affected by an environmental hazard by geography (e.g., census tract, ZCTA): `find_num_people_affected_by_geo`
   ...
   This function is very similar to `find_num_people_affected`. It differs in that it provides the output by an additional geography (e.g., ZCTAs, counties, census tracts). It provides the number of people affected, therefore, in each ZCTA, county, etc. As such, it has an additional input -- a geospatial data file for the desired geographic aggregation unit.
...
2. **Inputs**
   ...
   filetype: parquet or geojson (note: do not use parquet if you are using R)
...
This function reads in a climate hazard geospatial data file or spatial unit geospatial data file (counties, zcta, etc.) in parquet or geojson format that contains a string column with the geom ID and a geometry column, but nothing else. This function removes missing geometries and makes all geometries valid. It also reprojects to WGS84 and adds the best UTM zone for all hazard data (for buffering calculations).

- Input: dataframe with ONLY 2 columns -- `ID_climate_hazard` and `geometry`
- Output: dataframe is the same but has valid geometries, is in WGS84, and has UTM zones as applicable.

Step 3: `add_buffered_geo_col`
This function reprojects each hazard into the proper UTM zone and then adds a buffer distance to the provided polygons. The buffer distance is provided in the `buffer_dist` column. If the buffer distance is 0, the function does not buffer the geometry. After the buffer distance is added, the function reprojects the geometry back to WGS84.
...
- Output: dataframe with 4 columns
  ...
  4.  buffred hazard geometry (`buffered_hazard`)

Step 4: `prep_data`
This function uses `prep_geographies` and `add_buffered_geo_col` to prepare both the hazard data and the other geographical unit (e.g., ZCTA) data, if applicable, for the next steps.

- Input: geospatial data file for climate hazards and geospatial data file for the desired geographic aggregation unit (if applicable)
- Output: dataframe of the hazard data with 5 columns
  1.  climate hazard ID`ID_climate_hazard`
  2.  original hazard geometry (`geometry`)
  3.  best UTM projection (`XXXX`)
  4.  buffer distance (`buffer_dist`)
  4.  buffered hazard geometry (`buffered_hazard_geometry`)

Step 5: `combine_overlapping_geometries`
This function combines overlapping hazard geometries into a single geometry. The purpose is to avoid double counting people who are within the buffered area of two or more hazards. This function is only used if `by_unique_hazard=False`. If `by_unique_hazard=True`, this function is not used. When used, it retains an ID column that concatenates the IDs of the overlapping hazards.
- Input: prepared data that has been output from `prep_data`
- Output: dataframe with 3 columns
  1.  climate hazard ID (`ID_climate_hazard`)
  2.  original hazard geometry (`geometry`)

Step 6: `add_bounding_box_col`
...
- Input: prepared data that has been output from `prep_data` or `combine_overlapping_hazards` if applicable.
...
Step 7: `mask_raster_partial_pixel`
XXX
...
- Input: path to hazard data, path to population raster, path to additional geographis geospatial data file, `by_unique_hazard` argument
- Output: dataframe with 2 columns
  1.  climate hazard ID`ID_climate_hazard`
